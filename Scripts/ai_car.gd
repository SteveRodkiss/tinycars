...
#the ai will set these values to steer and move
var steer_input := 0.0
...
func _process(delta: float) -> void:
	if ai_control:
		return
	#steer_input = Input.get_axis("steer_right","steer_left") * 0.5
	#throttle_input = Input.get_axis("brake","accelerate")

func _physics_process(delta: float) -> void:
	#apply the input values to vehicle body
	steering = lerp(steering, steer_input,0.5)
	apply_engine_forces()
...
func get_velocity()->float:
	return (back_left_wheel.get_rpm() + back_right_wheel.get_rpm()) /2.0
